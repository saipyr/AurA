<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_aura_version ?? '0.0.0', AURA_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'AurA IDE version (v{{AURA_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})', {
						AURA_VERSION: AURA_VERSION,
						REQUIRED_VERSION: manifest?.required_aura_version ?? '0.0.0'
					}
				)
			);
			return;
