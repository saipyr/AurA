<script lang="ts">
	...
	import { config, models, settings, showSidebar, socket, user, AURA_NAME } from '$lib/stores';
	...
	const uploadFileHandler = async (file) => {
		...
		try {
			...
			if (uploadedFile) {
				...
				fileItem.url = `${AURA_API_BASE_URL}/files/${uploadedFile.id}`;
...
<svelte:head>
	<title>
		{note?.title
			? `${note?.title.length > 30 ? `${note?.title.slice(0, 30)}...` : note?.title} • ${$AURA_NAME}`
			: `${$AURA_NAME}`}
	</title>
</svelte:head>
