...
usage() {
    ...
    echo "  --aura[port=PORT]         Set the port for the web user interface."
    ...
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --aura[port=3000]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --aura[port=3000] --data[folder=./ollama-data]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --aura[port=3000] --data[folder=./ollama-data] --build"
    echo ""
...
if [[ $kill_compose == true ]]; then
    ...
    if [[ -n $aura_port ]]; then
        export OPEN_AURA_PORT=$aura_port # Set OPEN_AURA_PORT environment variable
    fi
...
echo -e "   ${GREEN}${BOLD}WebUI Port:${NC} $aura_port"
