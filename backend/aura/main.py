...
from open_aura.utils import logger
from open_aura.utils.audit import AuditLevel, AuditLoggingMiddleware
from open_aura.utils.logger import start_logger
from open_aura.socket.main import (
    app as socket_app,
    periodic_usage_pool_cleanup,
    get_models_in_use,
    get_active_user_ids,
)
from open_aura.routers import (
    audio,
    images,
    ollama,
    openai,
...
from open_aura.routers.retrieval import (
...
from open_aura.internal.db import Session, engine

from open_aura.models.functions import Functions
from open_aura.models.models import Models
from open_aura.models.users import UserModel, Users
from open_aura.models.chats import Chats

from open_aura.config import (
    LICENSE_KEY,
    # Ollama
    ENABLE_OLLAMA_API,
    OLLAMA_BASE_URLS,
...
from open_aura.env import (
    ...
    AURA_BUILD_HASH,
    AURA_SECRET_KEY,
    AURA_SESSION_COOKIE_SAME_SITE,
    AURA_SESSION_COOKIE_SECURE,
    AURA_AUTH_TRUSTED_EMAIL_HEADER,
    AURA_AUTH_TRUSTED_NAME_HEADER,
    AURA_AUTH_SIGNOUT_REDIRECT_URL,
...
from open_aura.utils.models import (
    get_all_models,
    get_all_base_models,
    check_model_access,
)
from open_aura.utils.chat import (
    generate_chat_completion as chat_completion_handler,
    chat_completed as chat_completed_handler,
    chat_action as chat_action_handler,
)
from open_aura.utils.embeddings import generate_embeddings
from open_aura.utils.middleware import process_chat_payload, process_chat_response
from open_aura.utils.access_control import has_access

from open_aura.utils.auth import (
    get_license_data,
...
from open_aura.utils.plugin import install_tool_and_function_dependencies
from open_aura.utils.oauth import OAuthManager
from open_aura.utils.security_headers import SecurityHeadersMiddleware
from open_aura.utils.redis import get_redis_connection

from open_aura.tasks import (
...
from open_aura.utils.redis import get_sentinels_from_env
...
{f"Commit: {AURA_BUILD_HASH}" if AURA_BUILD_HASH != "dev-build" else ""}
https://github.com/aura/aura
"""
)
...
if ENABLE_OTEL:
    from open_aura.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.AURA_URL = AURA_URL
...
app.state.config.BANNERS = AURA_BANNERS
...
app.state.AUTH_TRUSTED_EMAIL_HEADER = AURA_AUTH_TRUSTED_EMAIL_HEADER
app.state.AUTH_TRUSTED_NAME_HEADER = AURA_AUTH_TRUSTED_NAME_HEADER
app.state.AURA_AUTH_SIGNOUT_REDIRECT_URL = AURA_AUTH_SIGNOUT_REDIRECT_URL
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                "https://api.github.com/repos/aura/aura/releases/latest",
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for AurA IDE.
...
if len(OAUTH_PROVIDERS) > 0:
    app.add_middleware(
        ...
        same_site=AURA_SESSION_COOKIE_SAME_SITE,
        https_only=AURA_SESSION_COOKIE_SECURE,
    )
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            "name": app.state.AURA_NAME,
            "short_name": app.state.AURA_NAME,
            "description": "AurA IDE is an open, extensible, user-friendly interface for AI that adapts to your workflow.",
            "start_url": "/",
            "display": "standalone",
...
async def get_opensearch_xml():
    ...
    <ShortName>{app.state.AURA_NAME}</ShortName>
    <Description>Search {app.state.AURA_NAME}</Description>
    <InputEncoding>UTF-8</InputEncoding>
    <Image width="16" height="16" type="image/x-icon">{app.state.config.AURA_URL}/static/favicon.png</Image>
    <Url type="text/html" method="get" template="{app.state.config.AURA_URL}/?q={"{searchTerms}"}"/>
    <moz:SearchForm>{app.state.config.AURA_URL}</moz:SearchForm>
