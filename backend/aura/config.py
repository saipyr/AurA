...
from open_aura.internal.db import Base, get_db
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(OPEN_AURA_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = OPEN_AURA_DIR / "migrations"
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", OPEN_AURA_DIR / "static")).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.aura.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                AURA_FAVICON_URL = url = (
                    f"https://api.aura.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.aura.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (AurA IDE + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.aura.svc.cluster.local:11434"
...
AURA_URL = PersistentConfig("AURA_URL", "aura.url", os.environ.get("AURA_URL", ""))
...
try:
    banners = json.loads(os.environ.get("AURA_BANNERS", "[]"))
    banners = [BannerModel(**banner) for banner in banners]
except Exception as e:
    log.exception(f"Error loading AURA_BANNERS: {e}")
    banners = []

AURA_BANNERS = PersistentConfig("AURA_BANNERS", "ui.banners", banners)
...
QDRANT_COLLECTION_PREFIX = os.environ.get("QDRANT_COLLECTION_PREFIX", "aura")
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "open_aura_collections"
)
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "aura-index")
