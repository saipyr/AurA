...
from open_aura.models.auths import (
    AddUserForm,
    ApiKey,
    Auths,
    Token,
...
from open_aura.models.users import Users
from open_aura.models.groups import Groups

from open_aura.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
from open_aura.env import (
    AURA_AUTH,
    AURA_AUTH_TRUSTED_EMAIL_HEADER,
    AURA_AUTH_TRUSTED_NAME_HEADER,
    AURA_AUTH_TRUSTED_GROUPS_HEADER,
    AURA_AUTH_COOKIE_SAME_SITE,
...
from open_aura.config import OPENID_PROVIDER_URL, ENABLE_OAUTH_SIGNUP, ENABLE_LDAP
from pydantic import BaseModel

from open_aura.utils.misc import parse_duration, validate_email_format
...
from open_aura.utils.webhook import post_webhook
from open_aura.utils.access_control import get_permissions
...
async def get_session_user(
    ...
    if data:
        ...
        response.set_cookie(
            ...
            samesite=AURA_AUTH_COOKIE_SAME_SITE,
            secure=AURA_AUTH_COOKIE_SECURE,
        )
...
async def update_password(
    ...
    if AURA_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(request: Request, response: Response, form_data: SigninForm):
    if AURA_AUTH_TRUSTED_EMAIL_HEADER:
        if AURA_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[AURA_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if AURA_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(AURA_AUTH_TRUSTED_NAME_HEADER, email)
        ...
        user = Auths.authenticate_user_by_email(email)
        if AURA_AUTH_TRUSTED_GROUPS_HEADER and user and user.role != "admin":
...
async def signout(request: Request, response: Response):
                            ...
                            if logout_url:
                                ...
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f"&post_logout_redirect_uri={AURA_AUTH_SIGNOUT_REDIRECT_URL}"
                                            if AURA_AUTH_SIGNOUT_REDIRECT_URL
                                            else ""
                                        ),
                                    },
    ...
    if AURA_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                "status": True,
                "redirect_url": AURA_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def get_admin_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        "AURA_URL": request.app.state.config.AURA_URL,
...
async def update_admin_config(
    ...
    request.app.state.config.AURA_URL = form_data.AURA_URL
