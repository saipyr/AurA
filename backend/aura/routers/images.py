...
from open_aura.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from open_aura.routers.files import upload_file
from open_aura.utils.auth import get_admin_user, get_verified_user
from open_aura.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-AurA-User-Name"] = quote(user.name, safe=" ")
                headers["X-AurA-User-Id"] = user.id
                headers["X-AurA-User-Email"] = user.email
                headers["X-AurA-User-Role"] = user.role
