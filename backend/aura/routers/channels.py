...
from open_aura.socket.main import sio, get_user_ids_from_room
from open_aura.models.users import Users, UserNameResponse

from open_aura.models.channels import Channels, ChannelModel, ChannelForm
from open_aura.models.messages import (
    Messages,
    MessageModel,
    MessageResponse,
    MessageForm,
...
from open_aura.config import ENABLE_ADMIN_CHAT_ACCESS, ENABLE_ADMIN_EXPORT
from open_aura.constants import ERROR_MESSAGES
from open_aura.env import SRC_LOG_LEVELS


from open_aura.utils.auth import get_admin_user, get_verified_user
from open_aura.utils.access_control import has_access, get_users_with_access
...
async def send_notification(name, aura_url, channel, message, active_user_ids):
            ...
            if user.settings:
                ...
                if webhook_url:
                    post_webhook(
                        name,
                        webhook_url,
                        f"#{channel.name} - {aura_url}/channels/{channel.id}\n\n{message.content}",
                        {
                            "action": "channel",
                            "message": message.content,
                            "title": channel.name,
                            "url": f"{aura_url}/channels/{channel.id}",
                        },
                    )
    ...
    try:
        ...
        if message:
            ...
            background_tasks.add_task(
                ...
                request.app.state.config.AURA_URL,
                channel,
                message,
                active_user_ids,
            )
