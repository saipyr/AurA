...
from test.util.mock_user import mock_aura_user
...
class TestAuths(AbstractPostgresTest):
    ...
    def test_get_session_user(self):
        ...
        assert response.json() == {
            "id": "1",
            "name": "John Doe",
            "email": "john.doe@aura.com",
            "role": "user",
            "profile_image_url": "/user.png",
        }

    def test_update_profile(self):
        from open_aura.utils.auth import get_password_hash
        ...
        with mock_aura_user(id=user.id):
    ...
    def test_update_password(self):
        ...
        old_auth = self.auths.authenticate_user(
            "john.doe@aura.com", "old_password"
        )
        assert old_auth is None
        new_auth = self.auths.authenticate_user(
            "john.doe@aura.com", "new_password"
        )
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@aura.com", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@aura.com"
    ...
    def test_add_user(self):
        ...
        assert data["email"] == "john.doe2@aura.com"
