...
from open_aura.models.folders import Folders
...
from open_aura.routers.tasks import (
    generate_queries,
    generate_title,
    generate_follow_ups,
...
from open_aura.routers.retrieval import process_web_search, SearchForm
from open_aura.routers.images import (
    load_b64_image_data,
    image_generations,
    GenerateImageForm,
    upload_image,
)
from open_aura.routers.pipelines import (
    process_pipeline_inlet_filter,
    process_pipeline_outlet_filter,
)
from open_aura.routers.memories import query_memory, QueryMemoryForm

from open_aura.utils.webhook import post_webhook


from open_aura.models.users import UserModel
from open_aura.models.functions import Functions
from open_aura.models.models import Models

from open_aura.retrieval.utils import get_sources_from_items


from open_aura.utils.chat import generate_chat_completion
from open_aura.utils.task import (
    get_task_model_id,
    rag_template,
...
from open_aura.utils.tools import get_tools
from open_aura.utils.plugin import load_function_module_by_id
from open_aura.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)
from open_aura.utils.code_interpreter import execute_code_jupyter

from open_aura.tasks import create_task
...
from open_aura.constants import TASKS
                    ...
                    if not get_active_status_by_user_id(user.id):
                        ...
                        if webhook_url:
                            post_webhook(
                                ...
                                webhook_url,
                                f"{title} - {request.app.state.config.AURA_URL}/c/{metadata['chat_id']}\n\n{content}",
                                {
                                    "action": "chat",
                                    "message": content,
                                    "title": title,
                                    "url": f"{request.app.state.config.AURA_URL}/c/{metadata['chat_id']}",
                                },
                            )
